...
class AbsenceScreenState extends State<AbsenceScreen> {
  ...
  Future<void> getStudentsByClass(Classe selectedClass) async {
    Response response = await http.get(Uri.parse("http://10.0.2.2:8088/etudiant/getByClasseId/${selectedClass.codClass}"));
  ...
  Future<void> getAbsenceByStudentId() async {
    Response response = await http.get(Uri.parse("http://10.0.2.2:8088/absence/getByEtudiantId/${this.selectedStudent?.id}"));
    if (response.statusCode == 200) {
      List<dynamic> data = jsonDecode(response.body);

...
Future getAllStudent() async {
  Response response =
      await http.get(Uri.parse("http://10.0.2.2:8088/etudiant/all"));
  return jsonDecode(response.body);
}

Future deleteStudent(int id) {
  return http
      .delete(Uri.parse("http://10.0.2.2:8088/etudiant/delete?id=${id}"));
}

Future addStudent(Student student) async {
  print(student.dateNais);
  Response response =
      await http.post(Uri.parse("http://10.0.2.2:8088/etudiant/add"),
...
Future updateStudent(Student student) async {
  Response response =
      await http.put(Uri.parse("http://10.0.2.2:8088/etudiant/update"),
          headers: {"Content-type": "Application/json"},
          body: jsonEncode(<String, dynamic>{
            "id": student.id,
            "nom": student.nom,
...
Future<List<Student>> getStudentsByClassQuery(int id) async {
  Response response = await http.get(
    Uri.parse("http://10.0.2.2:8088/etudiant/getByClasseId"),
  );
